...
class unmix:
    ...
    def solve_ls(
        ...
        self.c = ss.linalg.splu((a.transpose() @ a+lambda_factor*ss.eye(a.shape[1])).tocsc())
    ...
    def solve_lr(
        ...
        objective = cp.Minimize(
            0.5*(cp.norm(l, "fro")**2 + cp.norm(r, "fro")**2)
        )
        constraints = [cp.norm(a @ l @ r - b,"fro") <= lambda_factor, 0 <= cp.vec(l), 0 <= cp.vec(r)]
        prob = cp.Problem(objective, constraints)
        result = prob.solve(verbose=verbose)
        self.c = c.value
    ...
    def solve_fcnnls(
        ...
        def cssls(CtC, CtA, Pset=None):
            ...
            if Pset is None or np.all(Pset): # for first iteration
                ...
                for k in range(len(breakIdx) - 1):
                    ...
                    Z[np.ix_(varsi,cols2solve)] = np.linalg.solve(CtC[np.ix_(varsi,varsi)], CtA[np.ix_(varsi,cols2solve)])
            return Z  
        ...
        def fcnnls(C: np.ndarray, A: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
            ...
            K[~Pset] = 0 # Set all values that are not in the passive set to 0 (comparable to NMF)
            D = K # Create a copy of K
            Fset = np.where(~np.all(Pset, axis=0))[0] # Find the columns that have some active members
            # Obtain the initial feasible solution and corresponding passive (Pset) and active set(Fset)

            e = 0
            while Fset.any() and e < maxiter_e: # while there are still columns with not all feasible solutions
                # Retry LS Solve for the columns that still have infeasible solutions
                K[:, Fset] = cssls(CtC, CtA[:, Fset], Pset[:, Fset])

                # Find any infeasible solutions
                Hset = np.where(np.any(K[:, Fset] < 0, axis=0))[0] # FInd in our new solution possible negative values
                if Hset.size > 0:
                    i = 0
                    ...
                    while Hset.size > 0 and i < maxiter:
                        ...
                        alpha[xi, xj] = D[xi, Hset[xj]] / (D[xi, Hset[xj]] - K[xi, Hset[xj]])
    ...
    def nnls_admm(
        ...
        # Pre calculate:
        pinv = np.linalg.inv(np.array((A.T@A+1*ss.eye(A.shape[1], dtype=np.float32)).todense())).astype(np.float32)
        AtY = np.array(A.T@Y)
